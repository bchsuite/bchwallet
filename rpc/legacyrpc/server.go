...
func (s *Server) handlerClosure(request *bchjson.Request) lazyHandler {
	s.handlerMu.Lock()
...
func sanitizeRequest(r *bchjson.Request) string {
...
// but this is only implemented here to work around an oddity with bchjson,
...
func (s *Server) invalidAuth(req *bchjson.Request) bool {
	cmd, err := bchjson.UnmarshalCmd(req)
	if err != nil {
		return false
	}
	authCmd, ok := cmd.(*bchjson.AuthenticateCmd)
	if !ok {
		return false
	}
	// Check credentials.
	...
	for {
		...
		case reqBytes, ok := <-wsc.allRequests:
			...
			default:
				...
				go func() {
					resp, jsonErr := f()
					mresp, err := bchjson.MarshalResponse(req.ID, resp, jsonErr)
					if err != nil {
...
func (s *Server) postClientRPC(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		resp, err := bchjson.MarshalResponse(req.ID, nil, bchjson.ErrRPCInvalidRequest)
		if err != nil {
			log.Errorf("Unable to marshal response: %v", err)
	...
	// Marshal and send.
	mresp, err := bchjson.MarshalResponse(req.ID, res, jsonErr)
	if err != nil {
		log.Errorf("Unable to marshal response: %v", err)
