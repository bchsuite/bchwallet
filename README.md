...
[![Build Status](https://travis-ci.org/bchsuite/bchwallet.png?branch=master)](https://travis-ci.org/bchsuite/bchwallet)
[![Build status](https://ci.appveyor.com/api/projects/status/88nxvckdj8upqr36/branch/master?svg=true)](https://ci.appveyor.com/project/jrick/bchwallet/branch/master)

bchwallet is a daemon handling bitcoin wallet functionality for a
...
can be found [here](https://github.com/bchsuite/bchd).  An alternative
...
https://github.com/bchsuite/bchwallet/releases
...
```
git clone https://github.com/bchsuite/bchwallet $GOPATH/src/github.com/bchsuite/bchwallet
cd $GOPATH/src/github.com/bchsuite/bchwallet
glide install
```
...
```
PS> cp "$env:ProgramFiles\Bchd Suite\Bchd\sample-bchd.conf" $env:LOCALAPPDATA\Bchd\bchd.conf
PS> cp "$env:ProgramFiles\Bchd Suite\Bchwallet\sample-bchwallet.conf" $env:LOCALAPPDATA\Bchwallet\bchwallet.conf
...
```
PS> cp $env:GOPATH\src\github.com\bchsuite\bchd\sample-bchd.conf $env:LOCALAPPDATA\Bchd\bchd.conf
PS> cp $env:GOPATH\src\github.com\bchsuite\bchwallet\sample-bchwallet.conf $env:LOCALAPPDATA\Bchwallet\bchwallet.conf
PS> $editor $env:LOCALAPPDATA\Bchd\bchd.conf
PS> $editor $env:LOCALAPPDATA\Bchwallet\bchwallet.conf
```
...
```bash
$ cp $GOPATH/src/github.com/bchsuite/bchd/sample-bchd.conf ~/.bchd/bchd.conf
$ cp $GOPATH/src/github.com/bchsuite/bchwallet/sample-bchwallet.conf ~/.bchwallet/bchwallet.conf
...
The [integrated github issue tracker](https://github.com/bchsuite/bchwallet/issues)
