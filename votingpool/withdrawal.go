...
// toMsgTx generates a bchwire.MsgTx with this tx's inputs and outputs.
...
func (p *Pool) StartWithdrawal(roundID uint32, requests []OutputRequest,
	...
	txStore *wtxmgr.Store, chainHeight int32, dustThreshold bchutil.Amount) (
...
func getRedeemScript(mgr *waddrmgr.Manager, addr *bchutil.AddressScriptHash) ([]byte, error) {
...
func signMultiSigUTXO(mgr *waddrmgr.Manager, tx *wire.MsgTx, idx int, pkScript []byte, sigs []RawSig) error {
	...
	redeemScript, err := getRedeemScript(mgr, addresses[0].(*bchutil.AddressScriptHash))
	if err != nil {
